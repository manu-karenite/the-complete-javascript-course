...
const solveProblem = function(age1) {
    ...
    const dogAgeToHuman = age1.map(currentAge => {
        let age = currentAge <= 2 ? (currentAge * 2) : (16 + currentAge * 4);
        return age;
    });
    ...
    const filtered_dogAgeToHuman = dogAgeToHuman.filter(currentAge => {
        if (currentAge >= 18)
            return currentAge;
    });
    ...
    const totalAge = filtered_dogAgeToHuman.reduce((sum, age, index, filtered_dogAgeToHuman) =>
        sum + age, 0);
